...
"""
Controller for atlas.html page.
"""
...
atlas_page = flask.Blueprint("atlas_page", __name__)
...
def param2ellipse(racen, deccen, radius, ba, phi, nellipse=100):
    ...
    ellipse = list(map(list, zip(*ellipse)))
    return ellipse
...
def image_info(image_file=None, racen=None, deccen=None,
               ...
               ext=0):
    image_fits = astropy.io.fits.open(image_file)
    image_hdr = image_fits[ext].header
...
@atlas_page.route('/atlas', methods=['GET'])
def atlas():
    """
    Controller for the atlas.html page
    """
    ...
    return render_template("atlas.html", **templateDict)
